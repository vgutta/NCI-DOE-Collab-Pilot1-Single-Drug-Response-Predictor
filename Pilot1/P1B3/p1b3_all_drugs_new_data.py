...
def load_cellline_expressions(path, dtype, ncols=None, scaling='std'):
    ...
    print("NCI exp dtypes:")
    print(nci_exp.dtypes)
...
def load_drug_descriptors(path, dtype, ncols=None, scaling='std'):
    ...
    return df_dg
    '''
    drug_desc = pd.read_csv('~/xgboost-single-drug-reponse-prediction/data/descriptors.2D-NSC.5dose.filtered.txt', sep='\t', engine='c',
                     na_values=['na','-',''],
                     converters ={'NAME' : str})
...
def load_dose_response(path, seed, dtype, min_logconc=-5., max_logconc=-5., subsample=None):
    ...
    nci60_filtered.rename(columns={'CELL':'CELLNAME', 'DRUG':'NSC'}, inplace=True)
    ...
    #fda_set = nci60_filtered
    
    df = nci60_filtered.dropna()

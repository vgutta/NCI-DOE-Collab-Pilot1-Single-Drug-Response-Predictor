...
def load_cellline_expressions(path, dtype, ncols=None, scaling='std'):
    ...
    df = df.loc[:, df.columns.isin(lincs_index) | df.columns.isin(['Sample'])]
    
    df.rename(columns={'Sample': 'CELLNAME'}, inplace=True)
...
def load_drug_descriptors(path, dtype, ncols=None, scaling='std'):
    ...
    df1.rename(columns={'NAME': 'CCLE'}, inplace=True)
...
def load_dose_response(path, seed, dtype, min_logconc=-5., max_logconc=-5., subsample=None):
    ...
    df.rename(columns={'CELL': 'CELLNAME', 'DRUG': 'CCLE'}, inplace=True)

    df = df[['CCLE', 'CELLNAME', 'AUC']]
    
    df = df.set_index(['CCLE'])
...
class DataLoader(object):
    ...
    def __init__(self, seed, dtype, val_split=0.2, test_cell_split=None, shuffle=True,
        ...
        for fea in self.drug_features:
            if fea == 'descriptors':
                self.df_drug_desc_ccle = load_drug_descriptors(drug_desc_path, dtype, ncols=feature_subsample, scaling=scaling)
                self.input_shapes['drug_descriptors'] = (self.df_drug_desc_ccle.shape[1] - 1,)
                df = df.merge(self.df_drug_desc_ccle[['CCLE']], on='CCLE')
        ...
        df_test = df.merge(df_test_cell, on='CELLNAME').merge(df_test_drug, on='CCLE')
        ...
        if scramble:
            ...
            self.df_response[['AUC']] = random_growth['AUC']
        ...
        self.input_dim = sum([np.prod(x) for x in self.input_shapes.values()]) - 1
